...
const Admin = require("../models/institutionSchema.js");
...
const adminRegister = async (req, res) => {
  try {
    ...
    const institutionName = req.body.institutionName || req.body.schoolName;

    const admin = new Admin({
      ...req.body,
      password: hashedPass,
      institutionName: institutionName,
      schoolName: institutionName, // Backward compatibility with old schema index
    });

    const existingAdminByEmail = await Admin.findOne({ email: req.body.email });
    const existingInstitution = await Admin.findOne({
      $or: [
        { institutionName: institutionName },
