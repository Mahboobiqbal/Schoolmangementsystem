...
const FacultyProgramDetails = () => {
  ...
  const LearnersButtonHaver = ({ row }) => {
    const options = ["Track Participation", "Provide Marks"];
    ...
    const handleParticipation = () => {
      navigate(`/Faculty/program/learner/participation/${row.id}/${moduleID}`);
    };
    const handleMarks = () => {
      navigate(`/Faculty/program/learner/marks/${row.id}/${moduleID}`);
    };
    ...
    return (
      <>
        <BlueButton
          variant="contained"
          onClick={() => navigate("/Faculty/program/learner/" + row.id)}
        >
          View
        </BlueButton>
        <React.Fragment>
        ...
        <>
          ...
          {getresponse ? (
            ...
            <Paper sx={{ width: "100%", overflow: "hidden" }}>
              ...
              {Array.isArray(programLearners) && programLearners.length > 0 && (
                <TableTemplate
...
export default FacultyProgramDetails;
